...
## models and results
results_rfe <- readRDS("../results/rfe.rds")
model_rfe_fit <- readRDS("../models/rfe_fit.rds")
results_rf_tests <- readRDS("../results/rf_tests.rds")
results_rf_trees <- readRDS("../results/rf_trees.rds")
results_rf_mtry <- readRDS("../results/rf_mtry.rds")
results_all <- readRDS("../results/all_models.rds")
...
In 10-fold cross-validation the dataset is divided into 10 equal parts, and the model is fit 10 times. In each iteration the model is fit to 9 parts of the data and the accuracy or error of the fit is measured on the 10^th^ part (validation set). There are 10 iterations, so that each of the 10 parts is used as the validation set. For a more detailed explanation of k-fold cross validation see (Hastie et al., 2009) pages 241-249 or (Kuhn & Johnson, 2018) pages 69-71. 
...
In order to select the best model for predicting waterpoint status, I followed the model selection guideline provided by Kuhn & Johnson (2018):
...
```{r extraction_type, fig.height=2.75}
...
```{r waterpoint_type, fig.height=2.75}
...
```{r amount_tsh2, fig.width=7, warning=F}
train %>% filter(amount_tsh > 0, !payment_type=="never pay", 
                 !payment_type=="other", !payment_type=="unknown") %>% 
  ggplot() +
...
```{r corr, fig.width=7.5, fig.height=7.5}
...
# Correlations for mixed feature types using cor2() function
...
results_rfe %>% ggplot(aes(Variables, Accuracy)) +
  geom_point(color="blue") +
  geom_line(color="blue") +
  geom_errorbar(aes(ymin=Accuracy-1.96*AccuracySD/sqrt(10), 
                    ymax=Accuracy+1.96*AccuracySD/sqrt(10)), width=0.5, color="grey50") +
...
varImpPlot(model_rfe_fit, main=NULL)
```
...
p_mtry <- results_rf_mtry %>% ggplot(aes(mtry, Accuracy)) +
  geom_point(color="blue") +
  geom_line(color="blue") +
  geom_errorbar(aes(ymin=Accuracy-1.96*AccuracySD/sqrt(10), 
                    ymax=Accuracy+1.96*AccuracySD/sqrt(10)), width=0.5, color="grey50") +
...
p_trees <- results_rf_trees %>% ggplot(aes(n_trees, accuracy)) +
  geom_point(color="blue") +
  geom_line(color="blue") +
  geom_errorbar(aes(ymin=accuracy-ci, ymax=accuracy+ci), color="grey50") +
  scale_y_continuous(limits=c(0.7635, 0.7735)) +
  ylab(NULL)
...
results_rf_tests %>% filter(!test_name %in% c("leak", "final", "final_500")) %>%
  ggplot(aes(test_name, accuracy)) +
  geom_point(color="blue") +
  geom_errorbar(aes(ymin=accuracy-ci, ymax=accuracy+ci), width=0.5, color="grey50") +
  xlab("feature substituted") 
...
results_rf_tests %>% filter(test_name %in% c("baseline", "final", "final_500", "leak")) %>%
  ggplot(aes(test_name, accuracy)) +
  geom_point(color="blue") +
  geom_errorbar(aes(ymin=accuracy-ci, ymax=accuracy+ci), width=0.2, color="grey50") +
...
```{r all_models, warning=F}
results_all %>% ggplot() +
  geom_point(aes(model, accuracy), color="blue", alpha=0.8) +
  geom_errorbar(aes(x=model, ymin=accuracy-ci, ymax=accuracy+ci), width=0.3, color="grey50") +
...
Hastie, T., Tibshirani, R., & Friedman, J. (2009). The Elements of Statistical Learning: Data Mining, Inference, and Prediction, Second Edition (2nd ed.). Retrieved from https://www.springer.com/us/book/9780387848570  
